    ...
    with open(fname) as f:
        ...
        for gauge in reader:
            if tuple(sorted(gauge.keys())) not in keysets:
                raise RuntimeError(
                    "Header in gauges look up table must be in {:}"
                    .format(list(keysets)))
...
def matchFlowacc(gauge, facc, max_distance, max_error, scaling_factor=1):
    ...
    bbox = {
        "ymin": y - max_distance, "ymax": y + max_distance,
        "xmin": x - max_distance, "xmax": x + max_distance,
    }
    grid = facc.shrink(**bbox).astype("float32")
    grid *= abs((grid.cellsize[0] * scaling_factor)
                * (grid.cellsize[1] * scaling_factor))
    ...
    while len(river_cells_y) == 0 and error < max_error:
        river_cells_y, river_cells_x = np.where(
            (grid.data >= size * (1 - error)) &
            (grid.data <= size * (1 + error)))
        error += .01

    if error <= max_error:
        # the closest river cell
        nn = ((river_cells_y - gauge_y_idx)**2 +
              (river_cells_x - gauge_x_idx)**2).argmin()
