...
class NcDimDataset(NcDataset):
    def __init__(self, fname, mode="r", ydim="y", xdim="x",
                 cellsize=None, y_shift=1, x_shift=0, *args, **kwargs):
    ...
    def cellsize(self):
        if self._cellsize is None:
            ...
            try:
                ...
                raise RuntimeError(
                    "Not enough coordinate values to infer cellsize")
    ...
    def shrink(self, ymin, ymax, xmin, xmax):
        ...
        def _removeCells(ymin, ymax, xmin, xmax):
            ...
            out = {
                ...
                "left": int(
                    np.floor((xmin - self.bbox["xmin"]) / cellsize[1])),
                "bottom": int(
                    np.floor((ymin - self.bbox["ymin"]) / cellsize[0])),
                "right": int(
                    np.floor((self.bbox["xmax"] - xmax) / cellsize[1])),
            }
            return out
        ...
        def _slices(cells):
            ...
            y = self.variables[self._y][:]
            ystart, yend = ((cells["bottom"], cells["top"])
                            if y[0] <= y[-1]
                            else (cells["top"], cells["bottom"]))
            yend = len(y) - yend

            x = self.variables[self._x][:]
            xstart, xend = ((cells["left"], cells["right"])
                            if x[0] <= x[-1]
        ...
        nc = NcDimDataset(None, "w", self._y, self._x,
                          self.cellsize, self.y_shift, self.x_shift)
        nc.copyAttributes(self.attributes)
        nc.copyDimensions(self.dimensions, skip=(self._y, self._x))
        nc.createDimensions(
            {k: (v.stop - v.start) for k, v in shrink_slices.items()})

        for name, var in self.variables.items():
            if any(d in var.dimensions for d in shrink_slices.keys()):
                slices = [shrink_slices.get(d, slice(None))
    ...
    def enlarge(self, ymin, ymax, xmin, xmax):
        ...
        def _padCells(ymin, ymax, xmin, xmax):
            ...
            out = {
                ...
                "bottom": int(
                    np.ceil((self.bbox["ymin"] - ymin) / cellsize[0])),
                "right": int(
                    np.ceil((xmax - self.bbox["xmax"]) / cellsize[1])),
            }
            return out
        ...
        def _coordinates(padding):
            ...
            out = {
                self._y: sorted(
                    ...
                    reverse=ycs < 0),
                self._x: sorted(
                    np.linspace(xmin, xmax, num=xnum, endpoint=True),
                    reverse=xcs < 0),
            }
            return out
        ...
        for name, var in self.variables.items():
            if any(d in var.dimensions for d in enlarge_slices.keys()):
                slices = [enlarge_slices.get(d, slice(None))
                          for d in var.dimensions]
                newvar = nc.copyVariable(var, data=False)
    ...
    def setMask(self, mask):
        ...
        for name, var in self.variables.items():
            ...
            if self._y in var.dimensions and self._x in var.dimensions:
                slices = [mask_slices.get(d, slice(None))
                          for d in var.dimensions]
